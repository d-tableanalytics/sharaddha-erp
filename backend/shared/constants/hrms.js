...
export const STORAGE_CATEGORIES = Object.freeze({
  ...
  /**
   * O2D order documents — PO copy, SOR/PI, payment proof, invoice, AWB/LR.
   *
   * In an HRMS constants file because the STORAGE LAYER is generic and these
   * tables are how it resolves a prefix, a ceiling and a URL lifetime; a
   * category it does not know is refused by `putObject`. The alternative was a
   * second storage stack for O2D, which would mean a second place to get
   * magic-byte sniffing and unguessable keys right — and the first one is
   * already correct.
   *
   * The AUTHORISATION is not shared. HRMS objects are authorised by HRMS
   * module/action/scope; an O2D document is authorised by the portal permission
