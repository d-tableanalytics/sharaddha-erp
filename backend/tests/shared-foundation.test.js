...
test('AD-7: every storage category has a namespaced key prefix', () => {
  for (const c of STORAGE_CATEGORY_LIST) {
    assert.ok(STORAGE_PREFIXES[c], `${c} needs a prefix`);
    // The rule is NAMESPACED — a leading domain segment then a path — not
    // literally "hrms/". That was the same thing while HRMS was the only thing
    // storing objects; O2D now shares the driver and namespaces itself under
    // `o2d/`. What must not change is that no category writes to the bucket
    // root, where keys from different domains could collide.
    assert.match(
      STORAGE_PREFIXES[c],
      /^[a-z0-9][a-z0-9-]*(\/[a-z0-9][a-z0-9-]*)+$/,
      `${c} prefix must be "<domain>/<path>" with no leading or trailing slash`,
    );
  }
});

test('AD-7: HRMS categories stay under hrms/, so the domains do not interleave', () => {
  const hrmsCategories = STORAGE_CATEGORY_LIST.filter(
    (c) => c !== STORAGE_CATEGORIES.O2D_DOCUMENT,
  );
  for (const c of hrmsCategories) {
    assert.ok(
      STORAGE_PREFIXES[c].startsWith('hrms/'),
      `${c} is an HRMS category and belongs under hrms/`,
    );
  }
  assert.ok(STORAGE_PREFIXES[STORAGE_CATEGORIES.O2D_DOCUMENT].startsWith('o2d/'));
});
