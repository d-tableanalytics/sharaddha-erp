...
/**
 * In the Customer Portal this asserted that mounting HRMS left the ERP's eight
 * mounts untouched. Here the assertion is the mirror image, and it is the one
 * that matters after the split: HRMS keeps its namespace UNCHANGED — so every
 * frontend service module and every bookmarked deep link still resolves — and
 * the app carries the shared identity surface and nothing else.
 *
 * The negative half is the point. A Customer Portal route reappearing here
 * would be a second writer against orders, stock or reservations on a shared
 * database, which is the failure mode the separation exists to prevent.
 */
test('the HRMS router is mounted in app.js under its own, unchanged namespace', async () => {
  const app = await src('../app.js');
  assert.match(app, /app\.use\('\/api\/v1\/hrms', hrmsRoutes\)/);

  // The shared identity surface: same paths, same controllers, same database.
  for (const p of ['auth', 'users', 'roles']) {
    assert.match(app, new RegExp(`app\\.use\\('/api/v1/${p}'`), `shared mount /${p} is missing`);
  }

  // Customer Portal business surface — must NOT be served from this process.
  for (const p of ['orders', 'reservations', 'notifications', 'sales', 'inventory', 'products', 'product-details']) {
    assert.doesNotMatch(
      app,
      new RegExp(`app\\.use\\('/api/v1/${p}'`),
      `/${p} belongs to the Customer Portal`,
    );
  }

  // And no legacy catch-all: the Customer Portal's `app.use('/api', apiRoutes)`
  // is what made a second, weaker login endpoint possible in the first place.
  assert.doesNotMatch(app, /app\.use\('\/api',/, 'no legacy catch-all router');
});
