...
/**
 * The Customer Portal's copy of this file asserts against its legacy
 * `routes/api.routes.js`, which once carried a second login endpoint doing a
 * plaintext comparison. That router is a Customer Portal artefact and was never
 * carried into this repository — so the assertion here is the stronger one it
 * was working towards: this repository has exactly ONE way in.
 *
 * Asserted over the whole tree rather than one named file, because the failure
 * being guarded against is a second endpoint appearing SOMEWHERE, and a test
 * that only reads the file it already knows about cannot see that.
 */
const toPosix = (p) => p.split(String.fromCharCode(92)).join('/');
const BLOCK_COMMENT = new RegExp('/[*][sS]*?[*]/'.replace('[sS]*?', '[' + String.fromCharCode(92) + 's' + String.fromCharCode(92) + 'S]*?'), 'g');
const LINE_COMMENT = new RegExp('(^|[^:])//.*$', 'gm');
const PLAINTEXT_COMPARE = new RegExp('user[.]password[ ]*!==[ ]*String[(]password[)]');
const LOGIN_MOUNT = new RegExp("router[.]post[(][ ]*'/login'");

test('there is exactly one login endpoint, and no raw-password comparison', async () => {
  ...
  const walk = async (dir) => {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      if (['node_modules', 'tests', 'uploads'].includes(entry.name)) continue;
  ...
  assert.equal(
    files.some((f) => toPosix(relative(root, f)) === 'routes/api.routes.js'),
    false,
    'the Customer Portal legacy /api router must not exist here',
  );
  ...
  for (const file of files) {
    ...
    assert.doesNotMatch(
      clean,
      PLAINTEXT_COMPARE,
      `${relative(root, file)} compares a password in the clear`,
    );
    if (LOGIN_MOUNT.test(clean)) {
      loginMounts.push(toPosix(relative(root, file)));
    }
  }
  ...
  assert.deepEqual(
    ...
    'login must be declared in exactly one place',
  );
});

test('the login route is rate limited', async () => {
  const authRoutes = await src('../modules/auth/auth.routes.js');
  assert.match(authRoutes, new RegExp("router[.]post[(]'/login',[ ]*loginLimiter,[ ]*login[)]"));
  assert.match(authRoutes, new RegExp("router[.]post[(]'/refresh',[ ]*refreshLimiter"));
