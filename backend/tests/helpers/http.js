 ...
 */
export function buildTestApp({ mount, verify }) {
  const app = express();
  // `verify` is how a signed-webhook route gets its raw bytes, exactly as
  // app.js supplies them in production. Optional, so every existing caller is
  // unaffected; without it a webhook test would exercise a parser the real
  // server does not use.
  app.use(express.json(verify ? { verify } : undefined));
  mount(app);
