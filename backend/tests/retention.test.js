...
/**
 * The Customer Portal scheduled this sweep unconditionally on its daily 00:00
 * cron. Here it is the same job at the same default time, behind an explicit
 * switch.
 *
 * The switch is the point, and it is why this test changed with the repository
 * split rather than being carried over verbatim. Both portals share ONE
 * database, and this sweep DELETES — audit rows, attendance selfies, employee
 * documents, inbox items. Two processes running it at the same minute is the
 * duplicate-execution hazard the split has to answer for, so the assertions
 * below pin all three properties that make it safe:
 *
 *   1. it is still node-cron on the same daily default, not a new scheduler
 *   2. it is GUARDED, so a deployment that has not yet taken ownership runs
 *      nothing
 *   3. the guard defaults to OFF, so forgetting to set it is the safe failure
 */
test('the sweep is scheduled on a guarded daily cron, not a new scheduler', async () => {
  const server = await src('../server.js');
  assert.match(server, /runHrmsRetentionSweep/);
  assert.match(server, /'0 0 \* \* \*'/, 'the daily midnight default must survive');
  assert.match(server, /cron\.schedule\(/, 'still node-cron');
  assert.match(server, /bootstrapHrms\(\)/, 'handlers must be registered at startup');

  // Guarded, and off unless a deployment opts in. `=== 'enabled'` rather than a
  // truthiness test: an unset variable, an empty string and a stray "false" all
  // have to mean "the Customer Portal still owns this job".
  assert.match(
    server,
    /process\.env\.HRMS_RETENTION_CRON === 'enabled'/,
    'the sweep must not run unless this deployment explicitly owns it',
  );
  assert.match(
    server,
    /if \(RETENTION_CRON_ENABLED\) \{\s*cron\.schedule\(/,
    'cron.schedule must sit inside the guard, not beside it',
  );
...
/**
 * The jobs that stayed behind.
 *
 * Every one of these WRITES against the shared database — settling purchase
 * orders, expiring reservations, claiming a report period. A second copy
 * running here would do all of it twice, so their absence is a property worth
 * asserting rather than a thing to remember.
 */
test('no Customer Portal background job runs in this process', async () => {
  // Comments stripped: server.js NAMES these jobs to explain why they are not
  // here, so matching the prose would fail on the very explanation of the rule.
  const server = (await src('../server.js'))
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/(^|[^:])\/\/.*$/gm, '$1');
  ...
  for (const job of [
    ...
    'runPoSettlement',
    'sweepUploads',
    'runWeeklyInventoryReport',
    'runWeeklyHistoryReport',
    'seedDefaultRoles',
    'seedInventoryDefaults',
    'seedAlertRules',
  ]) {
    assert.doesNotMatch(server, new RegExp(`\\b${job}\\b`), `${job} belongs to the Customer Portal`);
  }
});
