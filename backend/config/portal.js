...
/**
 * This repository is the Employee Portal.
 *
 * ⚠ THE ONE LINE THAT MUST DIFFER BETWEEN THE TWO REPOSITORIES.
 *
 * Every other file listed in SHARED-CONTRACT.md is byte-identical across both,
 * and `npm run verify:contract` checks that. This file is deliberately excluded
 * from that manifest: its whole job is to say which domain the deployment
 * serves, so two identical copies would mean one of them is lying.
 */
const DEFAULT_PORTAL = PORTALS.EMPLOYEE;
